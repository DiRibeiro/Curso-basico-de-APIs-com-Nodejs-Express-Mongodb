...
router.post ('/create', (req, res) => {
    ...
    Users.findOne({email}, (err, data) => {
        ...
        Users.create(req.body, (err, data) => {
            ...
            data.password = undefined; //Para não visualizar a senha
            return res.send(data);
        });
    }); 
});
...
router.delete ('/delete', (req, res) => {
    ...
    Users.findOne({email}, (err, data) => {
        if (err) return res.send({ error: "Erro na busca do usuário!" });
        // if (data) return res.send({ error: "Usuário não encontrado" });

        Users.deleteOne(req.body, (err, data) => {
            if (err) return res.send({ error: "Erro ao deletar usuário!"});
...
router.post ('/auth', (req, res) => {
    ...
    Users.findOne({email}, (err, data) => {
        ...
        bcrypt.compare(password, data.password, (err, same) => {
            ...
            data.password = undefined; //Para não vizualizar a senha
            return res.send(data);
        })
    }).select("+password");
});
